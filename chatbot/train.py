...
def test(params):
    ...
    """
    1.n_epoch是训练的轮次数
    2.理论上讲，训练轮次数越大，训练精度越高
    3.如果训练轮次数特别大，比如1000，有可能发生过拟合现象，但是否过拟合也和训练的数据有关
    4.n_epoch越大，训练的时间越长
    5.办公室电脑用P5000的GPU训练40轮，大概训练了3天，两轮大概一个半小时，如果使用CPU来训练，速度会更慢，可能一轮就要一小时
    """
    n_epoch = 2
    batch_size = 128
    ...
    with tf.Session(config=config) as sess:
        sess.run(init)
        model.pred.load(sess, save_path)
