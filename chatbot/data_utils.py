...
# 根据字典大小选择在GPU还是CPU上执行embedding操作
...
def batch_flow(data, ws, batch_size, raw=False, add_end=True):
    ...
    while True:
        ...
        batches = [np.asanyarray(x) for x in batches]
...
def batch_flow_bucket(data, ws, batch_size, raw=False, add_end=True, n_bucket=5, bucket_ind=1, debug=False):
    ...
    if isinstance(ws, (list, tuple)):
        assert  len(ws) == len(data), "len(ws)必须等于len(data), 如果ws是list or tuple"

    if isinstance(add_end, bool):
        add_end = [add_end] * len(data)
    else:
        assert (isinstance(add_end, (list, tuple))), "add_end 不是boolean，就应该是一个list(tuple) of boolean"
        assert  len(add_end) == len(data), "如果add_end是list(tuple)，那么add_end的长度应该和输入长度是一致的"
